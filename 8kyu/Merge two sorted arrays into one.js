...
function mergeArrays(arr1, arr2) {
  const mergedArray = [...arr1, ...arr2]; // Combine both arrays
  const sortedArray = mergedArray.sort((a, b) => a - b); // Sort in ascending order
  const result = [];

  for (const num of sortedArray) {
    if (result.length === 0 || num !== result[result.length - 1]) {
      result.push(num); // Add non-duplicate numbers to the result array
    }
  }
