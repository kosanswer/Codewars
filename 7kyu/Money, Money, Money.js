...
function calculateYears(principal, interest, tax, desired) {
  let years = 0; // Создаем переменную для хранения количества лет

  while (principal < desired) {
    // Пока текущая сумма меньше желаемой
    const accruedInterest = principal * interest; // Рассчитываем начисленные проценты
    const taxedInterest = accruedInterest * (1 - tax); // Применяем налог к процентам
    principal += taxedInterest; // Обновляем текущую сумму с учетом процентов и налога
    years++; // Увеличиваем счетчик лет на 1
  }
